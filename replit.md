...
- **Omie ERP Integration**: Synchronizes clients, vendors, products, overdue debts, and invoices hourly. Includes product mapping, vendor resolution, specific filters, stage extraction, fallback mechanisms, and automatic filtering of cancelled invoices. Customer table stores `omieClientCode` for bidirectional synchronization.
    - **Automatic Customer Registration (Oct 28, 2025)**: When creating a new customer in Integra with CPF/CNPJ, the system automatically attempts to register them in Omie ERP using the `createClient()` method. The process: (1) Normalizes CPF/CNPJ to digits-only format, (2) Checks if customer already exists in Omie to prevent duplicates, (3) Creates customer via `UpsertCliente` API call with all available data (name, fantasy name, email, phone, address, city, state, zip code), (4) Updates the Integra customer record with the returned `omieClientCode`, (5) Provides feedback via `_omieMessage` field without blocking local customer creation if Omie registration fails. This ensures seamless bidirectional synchronization and eliminates manual customer registration in Omie.
