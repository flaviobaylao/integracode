...
- **Financial Tracking**: Overdue debt monitoring and credit analysis. Automatic synchronization of overdue debts from Omie ERP 3 times daily (6am, 12pm, 3pm Brazil time) using `data_vencimento` field for accurate due date calculation and `valor_a_receber` for pending amounts. Debts are persisted in `overdue_debts` table for efficient order blocking without real-time API calls.
