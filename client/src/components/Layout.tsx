...
const MENU_CARD: Record<string, string> = {
  ...
  "driver-management": "Motoristas", "delivery-reports": "Relatórios de Entregas", "temperatura-entregas": "Temperatura das Entregas",
  ...
  billings: "Faturamentos", "fiscal-invoices": "Faturamento NF-e", "inutilizacao-nf": "Inutilização de NF", "billing-pipeline": "Pipeline Faturamento",
...
export default function Layout({ children, activeView, setActiveView, user }: LayoutProps) {
  ...
  const canAccessPainelAtendimento = !!user?.role && ['admin', 'administrative'].includes(user.role); // Painel de Atendimento: so administradores do sistema e administrativos
  ...
  const menuGroups: MenuGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'fluxo-entregas', label: 'Fluxo de Entregas', icon: 'fas fa-stream', available: canAccessReports || isVendedor, badge: null },
        ...
        { id: 'temperatura-entregas', label: 'Temperatura das Entregas', icon: 'fas fa-temperature-low', available: canAccessReports || canAccessIndustria, badge: null },
      ],
    },
    {
      ...
      items: [
        ...
        // A rota /cupons continua valendo para links salvos.
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'inutilizacao-nf', label: 'Inutilização de NF', icon: 'fas fa-ban', available: canAccessReports, badge: null },
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'gestao-debito-vendas', label: 'Débito e Variação de Vendas', icon: 'fas fa-wallet', available: canAccessReports || isVendedor || isTelemarketing, badge: null },
        { id: 'painel-atendimento', label: 'Painel de Atendimento', icon: 'fas fa-gauge-high', available: canAccessPainelAtendimento, badge: null },
      ],
    },
    {
  ...
  // nada para quem não tem configuração.
  const grantsView = (id: string) => {
    const card = MENU_CARD[id];
    return !!card && perms.hasConfig && perms.map?.[card]?.ver === true;
  };
  const roleFilterItems = (items: MenuItem[]) => {
    return items.filter(item => {
      const card = MENU_CARD[item.id];
      const roleOk = item.available
        && (!isMotorista || ['rota-entrega', 'entregas-do-dia', 'extrato-cliente'].includes(item.id))
        && (!isTelemarketing || TMK_WHITELIST.includes(item.id));
      // camada restritiva (inalterada): usuário configurado sem "ver" no card não vê
      const permOk = !card || perms.can(card, "ver");
      return (roleOk && permOk) || grantsView(item.id);
    });
  };
