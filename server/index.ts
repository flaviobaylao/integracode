...
import { registerPainelAtendimento } from "./painel-atendimento-routes";
import { registerPainelAtendimentoDigital } from "./painel-atendimento-digital";
import { registerPainelComunicacaoRoutes } from "./painel-comunicacao";
import { registerMktEnsaio } from "./mkt-ensaio";
...
import { initConferenciaAlerta } from "./conferencia-alerta";
...
run();
  ...
  registerMktEnsaio(app);
  registerCadastroReceitaSync(app);
  registerGeocodeAnalyze(app);
  registerReconciliation(app);
  initConferenciaAlerta(app); // Etapa 5 — alerta diario dos furos de gravidade alta
...
// VIGIA: configura numeros do gestor p/ notificacoes (CSV) em system_settings gestor_whatsapp
  ...
  app.post('/api/vendedor/fechamento/fechar', async (req: Request, res: Response) => {
    try {
      ...
      let pendReplies = Math.max(0, parseInt(String(b.pendingReplies), 10) || 0);
      // 🔔 Pendências do Inbox (17/set/2026): TODA réplica do admin sem resposta do vendedor —
      // inclusive de reports de dias anteriores — trava o fechamento, conferido no servidor.
      try {
        const { listarPendenciasInbox } = await import('./change-requests-routes');
        const { pendentes } = await listarPendenciasInbox(seller, date);
        pendReplies = Math.max(pendReplies, pendentes.length);
      } catch (_e: any) { console.warn('[FECHAR-ROTA] pendencias do inbox:', _e?.message); }
      if (pendReplies > 0) return res.status(400).json({ error: 'Ha ' + pendReplies + ' pendencia(s) do Inbox (questionamento do admin) sem resposta. Responda para fechar a rota.', pendingReplies: pendReplies });
  ...
  app.get('/api/admin/fechamento/mensal', authenticateUser, requireRole(['admin', 'coordinator', 'administrative']), async (req: Request, res: Response) => {
    try {
      ...
      const parseRegistros = (v: any): any[] => {
        let arr: any[] = [];
        try { arr = Array.isArray(v) ? v : (typeof v === 'string' ? JSON.parse(v) : []); } catch { arr = []; }
        return (arr || []).filter((r: any) => r && r.data).map((r: any) => ({ data: String(r.data), motivo: String(r.motivo || ''), obs: String(r.obs || '').trim() }));
      };
      const clientes = ((rc.rows || rc) as any[]).map((x: any) => ({ customerId: String(x.cid), sellerId: String(x.sid || ''), nome: x.nome || x.cid, cidade: x.cidade || '', periodicidade: String(x.periodicidade || ''), vendedor: x.vendedor || '', n: Number(x.n), motivo: x.motivo, obs: String(x.obs || '').trim(), registros: parseRegistros(x.registros), flagVisita: suspV.has(String(x.cid)), flagDebito: suspD.has(String(x.cid)) }));
...
// Limpeza (02/jul/2026): remove visitas PENDENTES (hoje+futuras) de clientes fora da lista de Clientes Ativos
  ...
  // mas NAO entra em rota/agenda de visitas. Idempotente. (redeploy limpo p/ recuperar servico apos churn de builds)
...
// AUDITORIA FISCAL (read-only): ultima NF por instancia/CNPJ/serie e cenarios fiscais — 1.0 (Neon) vs 2.0.
  app.get("/api/admin/fiscal/audit", authenticateUser, requireRole(['admin', 'coordinator', 'administrative']), async (_req, res) => {
    ...
    const out: any = { invoices_2_0: null, scenarios_2_0: null };
    try {
...
function up(){var f=document.getElementById('file').files[0];if(!f){show('Selecione o arquivo .p12',false);return;}var btn=document.getElementById('btn');btn.disabled=true;btn.textContent='Enviando...';var r=new FileReader();r.onload=function(){try{var bytes=new Uint8Array(r.result);var bin='';for(var i=0;i<bytes.length;i++)bin+=String.fromCharCode(bytes[i]);var b64=btoa(bin);fetch('/api/admin/pix/cert',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pfxBase64:b64,password:document.getElementById('pass').value})}).then(function(x){return x.json();}).then(function(j){btn.disabled=false;btn.textContent='Enviar certificado';if(j.ok){show('Certificado salvo com sucesso ('+j.savedBytes+' bytes, senha: '+(j.hasPassword?'sim':'nao')+'). Agora avise o Claude para validar a emissao de PIX.',true);}else{show('Erro: '+(j.error||JSON.stringify(j)),false);}}).catch(function(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro de rede: '+e.message,false);});}catch(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro: '+e.message,false);}};r.onerror=function(){btn.disabled=false;btn.textContent='Enviar certificado';show('Falha ao ler o arquivo',false);};r.readAsArrayBuffer(f);}
  ...
  app.get("/api/dashboard2/full", async (req, res) => {
    try {
      ...
      const blocked = await q2(`SELECT bo.id, COALESCE(c.name, '-') AS customer_name, TRIM(CONCAT(u.first_name, ' ', u.last_name)) AS seller_name, bo.total_amount, bo.block_reason, bo.blocked_at, COALESCE(scb.operation_type::text, bpb.operation_type, 'venda') AS operation_type FROM blocked_orders bo LEFT JOIN sales_cards scb ON scb.id = bo.sales_card_id LEFT JOIN LATERAL (SELECT operation_type FROM billing_pipeline b2 WHERE b2.sales_card_id = bo.sales_card_id ORDER BY created_at DESC LIMIT 1) bpb ON true LEFT JOIN customers c ON c.id = bo.customer_id LEFT JOIN users u ON (u.omie_vendor_code = bo.seller_id OR u.omie_vendor_code = replace(bo.seller_id,'omie-vendor-','') OR u.id = bo.seller_id) WHERE bo.status = 'blocked' AND COALESCE(scb.operation_type::text, bpb.operation_type, 'venda') !~* 'transf|cancel|amostr|troca'${SCOPE_BLK} ORDER BY bo.blocked_at DESC NULLS LAST`);
      const aFaturar = await q2(`SELECT bp.id, COALESCE(c.name, '-') AS customer_name, COALESCE(bp.seller_name, '') AS seller_name, bp.sale_value, bp.created_at FROM billing_pipeline bp LEFT JOIN customers c ON c.id = bp.customer_id WHERE bp.stage IN ('pedido','a_faturar') AND COALESCE(bp.operation_type::text, 'venda') !~* 'transf|cancel|amostr|troca' AND NOT EXISTS (SELECT 1 FROM billing_pipeline bl LEFT JOIN sales_cards scl ON scl.id = bl.sales_card_id LEFT JOIN sales_cards scb ON scb.id = bp.sales_card_id WHERE bl.stage::text = 'lixeira' AND bl.id <> bp.id AND (bl.sales_card_id = bp.sales_card_id OR scl.parent_card_id = bp.sales_card_id OR scb.parent_card_id = bl.sales_card_id OR (scl.parent_card_id IS NOT NULL AND scl.parent_card_id = scb.parent_card_id) OR (bl.customer_id = bp.customer_id AND bl.sale_value = bp.sale_value AND COALESCE(bl.updated_at, bl.created_at) >= bp.created_at))) AND NOT EXISTS (SELECT 1 FROM sales_cards scx WHERE scx.id = bp.sales_card_id AND LOWER(COALESCE(scx.status,'')) IN ('cancelled','canceled','cancelado'))${SCOPE_BP} ORDER BY bp.created_at DESC NULLS LAST`); // pedido na Lixeira nao conta como A faturar
  ...
  // 410 para as rotas antigas do Omie (E5) e do sync com o 1.0 (E6) que nao existem mais.
  // Fica DEPOIS de todas as rotas vivas (ex.: alias /api/omie/instances e
  // /api/omie/overdue-debts/cached) e ANTES do fallback do SPA, para que uma chamada antiga
  // receba JSON 410 e nao o index.html do SPA.
