...
- **Delivery & Route Optimization**:
    ...
    - **Admin Control**: Administrative users can manually add, delete, and optimize visits. **Empty Route Creation** (November 19, 2025): New endpoints allow creating empty routes for sellers without scheduled visits. POST `/api/daily-routes/create-empty` creates a completely empty route (works even without seller home coordinates by using default São Paulo coordinates), and POST `/api/daily-routes/generate` now accepts `allowEmpty: true` parameter to create empty routes when no visits are scheduled. This enables flexible manual route management where admins can add customers/leads to routes even without programmed visits. UI on `/rota-do-dia` displays "Nenhuma rota encontrada" message with two buttons: "Gerar Rota" (attempts automatic generation) and "Criar Rota Vazia" (creates empty route for manual population). UI on `/daily-routes` displays clear "Rota Vazia" message with button to add first customer when route has no visits.
...
- **Delivery Configuration Edit Validation & getPendingDeliveries Fix** (November 22, 2025): Fixed critical issue where pending deliveries (billings) were not being fully loaded in Gestão de Entregas. Root cause: query was excluding billings from customers not yet synchronized to Integra (only 3 customers: BARBOSA ESTUFA E BRASA, Restaurante Bendita Comida novo, CONDOMINIO MERCADINHO). Solution implemented: Modified getPendingDeliveries() to use fallback data from Omie billing records (customer_address, customer_latitude, customer_longitude) when customer JOIN fails, enabling display of all 66 pending deliveries instead of just 47. Query now: (1) Attempts JOIN with customer data if available, (2) Falls back to Omie data for address/coordinates if customer not in Integra, (3) Generates unique customerId (either customer ID or 'billing-{id}' fallback) for each billing. This maintains full delivery visibility while data synchronization occurs.
- **Evolution API WhatsApp Integration** (November 22, 2025): Created foundational WhatsApp integration service via Evolution API (`server/evolutionIntegration.ts`) supporting text messages, images with captions, message history retrieval, instance status checking, and template messages. Service auto-formats phone numbers to E.164 standard (55XXXXXXXXXXX). Ready for deployment after configuring environment variables: EVOLUTION_API_BASE_URL, EVOLUTION_API_KEY, EVOLUTION_INSTANCE_NAME. Backend routes for sending messages can be added on demand.
