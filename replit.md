...
- **WhatsApp Integration**: Evolution API (CHAT_HONEST instance) with webhook support for receiving messages, message sending, and real-time conversation tracking.
  - **IMPORTANT - Webhook URL Configuration**: In development mode, webhooks MUST use `REPLIT_DEV_DOMAIN` (not `REPLIT_DOMAINS`) because external traffic to `REPLIT_DOMAINS` routes to the Autoscale deployment, not the dev server. The system automatically detects the environment and configures the correct webhook URL.
