    ...
    try {
      ...
      const deliveries = await db.execute(sql`
        SELECT DISTINCT ON (b.id)
          ...
          COALESCE(c.exclusive_vehicle, sc.exclusive_vehicle, b.exclusive_vehicle, false) as "exclusiveVehicle",
          COALESCE(c.vehicle_types, sc.vehicle_types, to_jsonb(b.vehicle_types), '[]'::jsonb) as "vehicleTypes",
          ...
          COALESCE(c.delivery_weekdays, sc.delivery_weekdays, to_jsonb(b.delivery_weekdays), '[]'::jsonb) as "deliveryWeekdays",
          COALESCE(c.delivery_time_slots, sc.delivery_time_slots, to_jsonb(b.delivery_time_slots), '[]'::jsonb) as "deliveryTimeSlots",
          COALESCE(c.delivery_saturday_time_slots, sc.delivery_saturday_time_slots, to_jsonb(b.delivery_saturday_time_slots), '[]'::jsonb) as "deliverySaturdayTimeSlots"
        FROM billings b
