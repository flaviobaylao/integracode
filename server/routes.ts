...
const STAGE_DESCRIPTIONS: Record<string, string> = {
  ...
  '80': 'Aguardando Rota',
  '10': 'Pedido Incluído',
  '50': 'Separar Estoque / Faturar',
  '60': 'Faturado',
};

async function logOmieStageChange(params: {
  omieOrderId: number;
  orderNumber?: string;
  customerName?: string;
  ...
  try {
    await db.insert(omieStageLogs).values({
      omieOrderId: params.omieOrderId,
      orderNumber: params.orderNumber || null,
      customerName: params.customerName || null,
      previousStage: params.previousStage || null,
      newStage: params.newStage,
      stageDescription: STAGE_DESCRIPTIONS[params.newStage] || params.newStage,
      trigger: params.trigger,
      triggerDetail: params.triggerDetail || null,
      routeId: params.routeId || null,
      stopId: params.stopId || null,
      billingId: params.billingId || null,
      driverEmail: params.driverEmail || null,
      triggeredBy: params.triggeredBy || null,
      success: params.success,
      errorMessage: params.errorMessage || null,
      omieResponse: params.omieResponse || null,
    });
  } catch (err) {
    console.error('❌ [STAGE-LOG] Erro ao gravar log de etapa:', err);
  }
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // IMPORTANT: Specific routes must come BEFORE generic :entityType/:entityId routes
  ...
  app.get('/api/omie/stage-logs', authenticateUser, async (req: any, res) => {
    try {
      const { trigger, success, startDate, endDate, limit: queryLimit } = req.query;
      
      const conditions = [];
      
      if (trigger) {
        conditions.push(eq(omieStageLogs.trigger, trigger as string));
      }
      
      if (success !== undefined && success !== '') {
        conditions.push(eq(omieStageLogs.success, success === 'true'));
      }
      
      if (startDate) {
        conditions.push(gte(omieStageLogs.createdAt, new Date(startDate as string)));
      }
      
      if (endDate) {
        const end = new Date(endDate as string);
        end.setHours(23, 59, 59, 999);
        conditions.push(lte(omieStageLogs.createdAt, end));
      }
      
      const maxRows = Math.min(parseInt(queryLimit as string) || 200, 500);
      
      const logs = await db.select()
        .from(omieStageLogs)
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(desc(omieStageLogs.createdAt))
        .limit(maxRows);
      ...
      console.error('Erro ao buscar logs de etapas:', error);
  ...
  */
  ...
  app.get('/api/omie/vendedores', authenticateUser, async (req: any, res) => {
    try {
      ...
      const omieService = getOmieService(storage);
      if (!omieService) {
        console.log('❌ [VENDEDORES] OmieService não configurado');
        return res.status(503).json({ 
          message: "Integração Omie não configurada" 
        });
      }
      ...
      console.log(`✅ [VENDEDORES] Retornando ${vendedores.vendors.length} vendedores`);
      res.json(vendedores.vendors);

    } catch (error) {
      console.error("❌ [VENDEDORES] Error fetching vendors from Omie:", error);
      res.status(500).json({ 
      ...
      for (const routePlan of routes) {
        ...
        if (existingRoutes.length > 0) {
          ...
          if (billingIds.length > 0) {
            ...
            }).from(billingsTable).where(inArray(billingsTable.id, billingIds));
          ...
          const stopsData = stops.map((stop: any, index: number) => ({
            salesCardId: stop.salesCardId,
            billingId: stop.billingId || null,
            orderNumber: (stop.billingId && billingInfoMap[stop.billingId]?.orderNumber) || stop.orderNumber || null,
            omieOrderId: (stop.billingId && billingInfoMap[stop.billingId]?.omieOrderId) || null,
            customerId: stop.customerId,
            customerName: stop.customerName,
            customerAddress: stop.customerAddress,
            customerLatitude: parseFloat(stop.latitude) || 0,
            ...
            estimatedDeparture: stop.estimatedDeparture || null,
            estimatedServiceTime: parseInt(stop.estimatedServiceTime) || 30,
            distanceFromPrevious: parseFloat(stop.distanceFromPrevious) || 0,
            isPriority: stop.isUrgent || false,
            status: 'pendente'
  ...
  app.post("/api/delivery-routes/sync-omie-stages-day", authenticateUser, requireRole(['admin', 'coordinator', 'administrative']), async (req: any, res) => {
    try {
      ...
      console.log(`🔄 [SYNC-OMIE-DAY] Sincronizando etapas de TODAS as rotas do dia ${date} para "Em Rota" (20)`);
      
      const routes = await db.select().from(deliveryRoutes)
        .where(and(
          sql`${deliveryRoutes.routeDate}::text = ${date}`,
          inArray(deliveryRoutes.status, ['rota_salva', 'rota_enviada', 'em_andamento'])
        ));
      
      if (routes.length === 0) {
        return res.json({ message: "Nenhuma rota encontrada para esta data", successCount: 0, errorCount: 0, results: [] });
      }
      ...
      const orderInfos: Array<{ orderId: number; orderNumber: string; customerName: string; stopId: string; routeId: string; billingId: string; driverEmail: string }> = [];
      
      for (const stop of allStops) {
        let omieId = stop.omieOrderId ? parseInt(stop.omieOrderId) : NaN;
        
        if (isNaN(omieId) && stop.billingId) {
          const billing = await db.select().from(billingsTable)
            .where(eq(billingsTable.id, stop.billingId))
            .limit(1);
          if (billing.length > 0 && billing[0].omieOrderId) {
            omieId = parseInt(billing[0].omieOrderId);
            if (!isNaN(omieId)) {
              await db.update(deliveryRouteStops)
                .set({ 
                  omieOrderId: billing[0].omieOrderId,
                  orderNumber: billing[0].orderNumber || stop.orderNumber,
                })
                .where(eq(deliveryRouteStops.id, stop.id));
            }
          }
        }
      ...
      if (orderInfos.length === 0) {
        return res.json({ message: "Nenhum pedido Omie encontrado nas rotas", successCount: 0, errorCount: 0, results: [] });
      }
  ...
  // VERSÃO SIMPLIFICADA: Usa algoritmo nearest neighbor sem 2-opt para evitar loops
  app.post("/api/delivery-routes/:routeId/optimize", authenticateUser, requireRole(['admin', 'coordinator', 'administrative', 'motorista']), async (req: any, res) => {
    try {
      ...
      console.log(`📍 [OPTIMIZE-ROUTE] ${stops.length} paradas encontradas`);
      ...
      const destinations = stops.filter(stop => {
        ...
        isUrgent: stop.isPriority === true,
        priority: stop.isPriority ? 5 : 3,
        estimatedDuration: stop.estimatedServiceTime || 30
      }));
      ...
      console.log(`🗺️ [OPTIMIZE-ROUTE] Otimizando ${destinations.length} destinos...`);
      
      // ALGORITMO SIMPLIFICADO: Nearest Neighbor direto sem 2-opt
      // Separa urgentes primeiro, depois otimiza por distância
      const urgentDestinations = destinations.filter(d => d.isUrgent);
      const normalDestinations = destinations.filter(d => !d.isUrgent);
      ...
      // Nearest neighbor para um conjunto de destinos
      const optimizeByNearestNeighbor = (dests: typeof destinations, start: { latitude: number, longitude: number }) => {
        if (dests.length === 0) return [];
        const result: typeof destinations = [];
        ...
        while (remaining.length > 0) {
          ...
          for (let i = 0; i < remaining.length; i++) {
            ...
            if (dist < nearestDist && !isNaN(dist)) {
              nearestDist = dist;
              nearestIdx = i;
            }
          }
      ...
      // Otimizar urgentes primeiro (a partir do ponto de início)
      const optimizedUrgent = optimizeByNearestNeighbor(urgentDestinations, startLocation);
      
      // Otimizar normais (a partir do último urgente ou ponto de início)
      const lastUrgent = optimizedUrgent[optimizedUrgent.length - 1];
      const normalStart = lastUrgent 
        ? { latitude: lastUrgent.latitude, longitude: lastUrgent.longitude }
        : startLocation;
      const optimizedNormal = optimizeByNearestNeighbor(normalDestinations, normalStart);
      
      // Combinar resultados
      const optimizedLocations = [...optimizedUrgent, ...optimizedNormal];
      ...
      for (const loc of optimizedLocations) {
        totalDistance += haversineDistance(currentLat, currentLng, loc.latitude, loc.longitude);
        currentLat = loc.latitude;
        currentLng = loc.longitude;
      }
      
      console.log(`✅ [OPTIMIZE-ROUTE] Otimização concluída: ${Math.round(totalDistance)}m`);
      
      // Atualizar ordem das paradas no banco
      for (let i = 0; i < optimizedLocations.length; i++) {
        await db.update(deliveryRouteStops)
          .set({ stopOrder: i + 1 })
          .where(eq(deliveryRouteStops.id, optimizedLocations[i].id));
      }
      
      // Atualizar distância e duração da rota
      const totalDistanceKm = totalDistance / 1000;
      const totalDurationMinutes = Math.round((totalDistance / 1000 / 30) * 60 + optimizedLocations.length * 15);
      ...
      console.log(`✅ [OPTIMIZE-ROUTE] Rota ${routeId} otimizada: ${totalDistanceKm.toFixed(2)}km, ${totalDurationMinutes}min`);
  ...
  app.get('/api/daily-routes/:sellerId/date/:date', authenticateUser, async (req: any, res) => {
    try {
      ...
      // Query protegida por try-catch para não quebrar visualização da rota
      let ordersCount = 0;
      try {
        ...
        // Não propagar erro - continuar com ordersCount = 0
      }
  ...
  app.get('/api/leads-debug', async (req: any, res) => {
    ...
    try {
      ...
      // Testando LIMIT 10 (minimal)
      const result = await db.execute(sql`SELECT * FROM leads ORDER BY created_at DESC LIMIT 10`);
  ...
  app.post('/api/chat/sync-whatsapp', async (req: any, res) => {
    ...
    try {
      // Buscar todas as conversas do banco de dados
      const allConversations = await storage.getChatConversations();
      console.log(`📊 [SYNC-WHATSAPP] ${allConversations.length} conversas no banco de dados`);
      
      // Retornar conversas atualizadas
      res.json({ 
        success: true, 
        message: `Sincronização concluída: ${allConversations.length} conversas`,
        totalChats: allConversations.length,
        syncedChats: allConversations.length
      });

      console.log(`✅ [SYNC-WHATSAPP] ===== SINCRONIZAÇÃO CONCLUÍDA =====`);
    } catch (error: any) {
      console.error('❌ [SYNC-WHATSAPP] ERRO:', error.message);
      res.status(500).json({ 
        success: false, 
