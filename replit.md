...
- **Visit Schedule Management**: Implemented a permanent sales card architecture where each active customer has one reusable `sales_card`. All visit results are stored in `order_history`. `nextVisitDate` is calculated from customer's `weekdays` and `visitPeriodicity`. Updates to customer weekdays or periodicity trigger recalculation of `nextVisitDate`. Route generation queries for `isPermanent=true` and `nextVisitDate` within the current day using **Brazil timezone (America/Sao_Paulo)** for accurate date matching.
  - **Card Initialization (Nov 2025)**: Created `scripts/initialize-permanent-cards.ts` to backfill permanent cards for all active customers. Successfully populated 810 permanent cards for eligible customers (813 total eligible, 3 without assigned seller). Script is idempotent and validates: seller assignment, coordinates, weekdays configuration, and visit periodicity. Cards use `calculateNextVisitDate()` from `shared/visitSchedule.ts` with proper Brazil timezone handling.
  - **Route Regeneration Fix (Nov 12, 2025)**: Fixed critical bug where "Atualizar Rota" button cleared all visits from map. Root cause: regeneration endpoint used deprecated `getSalesCardsByDate(scheduledDate)` incompatible with permanent cards (which use `nextVisitDate`). Solution: Created `planDailyRoute()` helper function that separates route planning from database persistence, then refactored regeneration to use `planDailyRoute() + updateDailyRoute()` pattern. This preserves route ID, checkpoints, and progress while updating visit order. Note: `optimizedOrder` contains customer IDs (not salesCard IDs) as confirmed by system architecture.
