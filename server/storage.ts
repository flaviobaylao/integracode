...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByEmail(email: string): Promise<User | undefined> {
    // Busca case-insensitive e tolerante a espaços: evita falha de login quando o
    // email é digitado com maiúsculas (autocapitalize de celular) ou espaços, e
    // também casa emails gravados com maiúsculas. Sem duplicidade por caixa no banco.
  ...
  async getPendingDeliveries(): Promise<PendingDelivery[]> {
    ...
    return result.rows.map((row: any) => {
      ...
      return {
        ...
        customerRazaoSocial: row.customerRazaoSocial || '',
  ...
  async getOverdueDebtByDocument(document: string): Promise<any | undefined> {
    ...
    if (n > 0) {
      console.log(`ℹ️ [STORAGE] Débito vencido de R$ ${saldoNum.toFixed(2)} dentro da tolerância (<= R$ ${DEBT_BLOCK_TOLERANCE}) — não bloqueia: doc ${normalizedSearchDocument}`);
    } else {
  ...
  async createPixCharge(data: InsertPixCharge): Promise<PixCharge> {
    ...
    // 📨 ENVIO AUTOMATICO (21/ago/2026): a cobranca PIX acabou de nascer. O gatilho
    // e o ponto central porque todos os provedores (BB e Inter) passam por aqui.
    // Fire-and-forget: se o cliente nao tiver canal marcado, o gatilho apenas sai.
  ...
  async getReceivables(filters?: { customerId?: string; status?: string; instanceId?: string; startDate?: Date; endDate?: Date; dueDateStart?: Date; dueDateEnd?: Date; paymentMethod?: string; chartAccountId?: string }): Promise<Receivable[]> {
    ...
    // Recomputa o status EXIBIDO de títulos EM ABERTO por DIA-CALENDÁRIO (fuso Brasil),
    // nas DUAS direções: a_vencer com vencimento ANTERIOR a hoje => vencida; e vencida
