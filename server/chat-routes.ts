...
export function registerChatRoutes(app: Express): void {
  ...
  // Retomada de contato: o unico caminho legitimo para falar fora da janela de 24h.
  try { registerRetomada(app); }
  catch (e: any) { console.error('[RETOMADA] nao registrado:', e?.message || e); }
    ...
    // 🔍 DEBUG: Registrar todos os dados para diagnóstico
      ...
      try {
        if (!isFromMe && String((conversation as any).status || '') === 'resolved') {
          ...
          //
          // O DONO, porem, fica. Zerar o assignedAgentId aqui era o que obrigava o atendente
          // a se atribuir a cada mensagem: sem dono, o webhook mandava a conversa para o
          // round-robin, que a entregava a outra pessoa — e quem estava conversando com o
          // cliente batia na trava "atendida por outro atendente". Quem ja estava na conversa
          // continua nela; so quando o dono era a IA (ou nao havia dono) a conversa e liberada.
          const donoAtual = String((conversation as any).assignedAgentId || '');
          const manterDono = !!donoAtual && donoAtual !== 'chatgpt';
          const patch: any = { status: 'new', initiatedBy: 'customer', initiatedByUserId: null };
          if (!manterDono) { patch.assignedAgentId = null; patch.assignedAgentColor = null; }
          await storage.updateChatConversation(conversation.id, patch);
          (conversation as any).status = 'new';
          if (!manterDono) (conversation as any).assignedAgentId = null;
          (conversation as any).initiatedBy = 'customer';
          try { const { liberarIA } = await import('./ia-fila'); await liberarIA(conversation.id); } catch {}
          console.log(`🔄 [WEBHOOK] Conversa ${conversation.id} REABERTA como conversa de ENTRADA` + (manterDono ? ` (segue com ${donoAtual})` : ' (sem dono)'));
        }
...
// 1841: etiqueta de canal + janela 24h + opt-out (canal oficial)
  ...
  // DIAGNÓSTICO 2 (read-only): inspeciona a API do Umbler — paginacao, canais e sinais de
  // "disparo de robo" vs mensagem de atendente. ?phone=55... foca num contato (chats distintos).
  app.get("/api/chat/umbler-talk/inspect", async (req: any, res: any) => {
    try {
      const cfg = await resolveUmblerTalkConfig();
      if ('error' in cfg) return res.status(400).json({ error: cfg.error });
      const org = encodeURIComponent(cfg.orgId);
      const getItems = (b: any) => Array.isArray(b) ? b : ((b && (b.items || b.Items)) || []);
      const p1 = await umblerTalkFetch('/v1/chats/?organizationId=' + org + '&page=1').then((r: any) => r.json()).catch(() => null);
      const p2 = await umblerTalkFetch('/v1/chats/?organizationId=' + org + '&page=2').then((r: any) => r.json()).catch(() => null);
      const i1 = getItems(p1), i2 = getItems(p2);
      const idOf = (c: any) => c && (c.id || c.Id);
      const bodyKeys = (p1 && !Array.isArray(p1)) ? Object.keys(p1) : ['<array>'];
      const pageAdvances = (i1[0] && i2[0]) ? (idOf(i1[0]) !== idOf(i2[0])) : null;
      const sig = (m: any) => {
        if (!m || typeof m !== 'object') return null;
        const g = (a: string, b: string) => (m[a] !== undefined ? m[a] : m[b]);
        const sbomRaw = g('SentByOrganizationMember', 'sentByOrganizationMember');
        return {
          source: g('Source', 'source'),
          messageType: g('MessageType', 'messageType'),
          fromContact: g('FromContact', 'fromContact'),
          templateId: !!(m.templateId || m.TemplateId),
          bulkSend: !!(m.bulkSendSession || m.BulkSendSession),
          botInstance: !!(m.botInstance || m.BotInstance),
          scheduled: !!(m.scheduledMessage || m.ScheduledMessage),
          aiCredits: g('deductedAiCredits', 'DeductedAiCredits') || 0,
          sbom: (sbomRaw && typeof sbomRaw === 'object') ? (sbomRaw.id || sbomRaw.Id) : sbomRaw,
          preview: String(m.content || m.Content || '').slice(0, 30),
        };
      };
      const chanName = (c: any) => { const ch = c.channel || c.Channel || {}; return ch.name || ch.Name || ch.phoneNumber || ch.PhoneNumber || '?'; };
      const chanCount: any = {}, samplesByChan: any = {};
      for (const c of i1) { const k = chanName(c); chanCount[k] = (chanCount[k] || 0) + 1; if (!samplesByChan[k]) samplesByChan[k] = sig(c.lastMessage || c.LastMessage); }
      const out: any = { orgId: cfg.orgId, bodyKeys, pageAdvances, page1Count: i1.length, page2Count: i2.length, page1First: idOf(i1[0]), page2First: idOf(i2[0]), chanCount, samplesByChan };
      // Foco num contato: chats distintos (dedupe por id) + sinais por mensagem-marco
      const phone = String(req.query.phone || '');
      if (phone) {
        const core = _phoneCore(phone);
        const seen = new Set<string>(); const chats: any[] = [];
        for (const c of i1) { const ct = c.contact || c.Contact || {}; const cp = ct.phoneNumber || ct.PhoneNumber || ct.Phone || ''; if (_phoneCore(cp) === core) { const id = String(idOf(c)); if (!seen.has(id)) { seen.add(id); chats.push(c); } } }
        out.contact = {
          core, distinctChats: chats.length,
          chats: chats.map((c: any) => ({
            chatId: idOf(c), channel: chanName(c), eventAtUTC: c.eventAtUTC || c.EventAtUTC,
            firstContact: sig(c.firstContactMessage || c.FirstContactMessage),
            firstMemberReply: sig(c.firstMemberReplyMessage || c.FirstMemberReplyMessage),
            last: sig(c.lastMessage || c.LastMessage),
          })),
        };
      }
      return res.json(out);
      ...
      if (storage.updateChatConversation) {
        ...
        if (userAgent) {
          ...
          if (isFirstAssignment) {
            ...
            // "Iniciada pelo atendente" e so quando o CLIENTE nunca escreveu nesta conversa.
            // Antes, responder a um cliente que chegou sozinho marcava a conversa como do
            // atendente — e, com a regra de origem, a IA nunca mais entrava naquele numero.
            let clienteJaFalou = false;
            try {
              const _cf: any = await db.execute(sql`SELECT 1 FROM chat_messages
                WHERE conversation_id = ${conversation.id} AND sender_type = 'customer' LIMIT 1`);
              clienteJaFalou = !!_cf.rows?.length;
            } catch {}
            await storage.updateChatConversation(conversation.id, clienteJaFalou
              ? { status: 'in-progress' } as any
              : { status: 'in-progress', initiatedBy: 'user', initiatedByUserId: userId } as any);
