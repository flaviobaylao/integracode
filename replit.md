...
- **Automated Agenda Management**: Scheduled daily (midnight UTC-3) and on-demand synchronization of sales cards for the next two months, calculating correct visit dates, managing cards, and providing statistics.
    - **Visit Allocation Bug Fix (Nov 2025)**: Fixed critical timezone conversion bug in `getRouteDay()` function that caused -1 day offset (e.g., Tuesday visits allocated to Monday). Removed problematic `toLocaleString()` conversion in favor of direct `getDay()` call. Affected functions: `getRouteDay`, `normalizeToBrazilDate`, `getBrazilToday`, `getStartOfDayBrazil` in `server/visitScheduleService.ts`.
