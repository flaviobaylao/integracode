...
import { TransferConversationModal } from "@/components/transfer-conversation-modal";
...
export function ChatArea({ conversation, currentUser }: ChatAreaProps) {
  ...
  const { data: conversationData, isLoading } = useQuery<{ messages: MessageWithSender[] }>({
    queryKey: ["/api/conversations", conversation.id],
    refetchInterval: 2000, // Refresh every 2 seconds for new messages
  });

  const messages: MessageWithSender[] = conversationData?.messages || [];
