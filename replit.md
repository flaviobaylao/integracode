...
- **Automated Agenda Management**: Scheduled daily (midnight UTC-3) and on-demand synchronization of sales cards for the next two months, calculating correct visit dates, managing cards, and providing statistics.
    ...
    - **Multiple Weekdays Support (Nov 2025)**: System supports customers with multiple weekdays configured (e.g., `["Seg", "Ter", "Qua", "Qui", "Sex"]`). The CustomerModal interface allows unlimited weekday selection via checkbox buttons. When scheduling visits for customers with multiple days, `calculateNextVisitDate()` automatically selects the nearest available day from the configured list, ensuring flexible visit planning.
