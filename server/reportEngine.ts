...
const DATA_SOURCES: Record<string, DataSourceDef> = {
  ...
  // NOTA: aqui a deduplicacao segue por access_key, e NAO por (CNPJ, serie, numero) como na
  // Regra Oficial (server/faturamento-oficial.ts). O construtor de relatorios e ferramenta de
  // exploracao: precisa continuar mostrando canceladas, devolvidas e rejeitadas. Para o numero
  // OFICIAL de faturamento use a regra, nao esta fonte.
  fiscal_invoices: {
    key: 'fiscal_invoices',
    label: 'Notas Fiscais (NF-e)',
    description: 'Todas as NF-e emitidas no Integra com dados fiscais completos',
    baseQuery: `SELECT fi.* FROM (SELECT DISTINCT ON (COALESCE(access_key, id)) * FROM fiscal_invoices ORDER BY COALESCE(access_key, id), created_at DESC) fi LEFT JOIN sales_cards sc ON sc.id = fi.sales_card_id LEFT JOIN (SELECT DISTINCT ON (inv_num) inv_num, bp_seller_id FROM (SELECT NULLIF(regexp_replace(COALESCE(invoice_number,''),'[^0-9]','','g'),'')::bigint AS inv_num, seller_id AS bp_seller_id, created_at FROM billing_pipeline WHERE stage <> 'lixeira' AND regexp_replace(COALESCE(invoice_number,''),'[^0-9]','','g') <> '') t ORDER BY inv_num, created_at DESC) bp ON bp.inv_num = fi.invoice_number LEFT JOIN LATERAL (SELECT TRIM(COALESCE(u.first_name, '') || ' ' || COALESCE(u.last_name, '')) AS seller_name FROM users u WHERE u.id = COALESCE(NULLIF(bp.bp_seller_id,''), sc.seller_id) OR u.omie_vendor_code = COALESCE(NULLIF(bp.bp_seller_id,''), sc.seller_id) OR u.omie_vendor_code = REPLACE(COALESCE(NULLIF(bp.bp_seller_id,''), sc.seller_id,''),'omie-vendor-','') LIMIT 1) u ON true`,
    fields: [
