...
function up(){var f=document.getElementById('file').files[0];if(!f){show('Selecione o arquivo .p12',false);return;}var btn=document.getElementById('btn');btn.disabled=true;btn.textContent='Enviando...';var r=new FileReader();r.onload=function(){try{var bytes=new Uint8Array(r.result);var bin='';for(var i=0;i<bytes.length;i++)bin+=String.fromCharCode(bytes[i]);var b64=btoa(bin);fetch('/api/admin/pix/cert',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pfxBase64:b64,password:document.getElementById('pass').value})}).then(function(x){return x.json();}).then(function(j){btn.disabled=false;btn.textContent='Enviar certificado';if(j.ok){show('Certificado salvo com sucesso ('+j.savedBytes+' bytes, senha: '+(j.hasPassword?'sim':'nao')+'). Agora avise o Claude para validar a emissao de PIX.',true);}else{show('Erro: '+(j.error||JSON.stringify(j)),false);}}).catch(function(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro de rede: '+e.message,false);});}catch(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro: '+e.message,false);}};r.onerror=function(){btn.disabled=false;btn.textContent='Enviar certificado';show('Falha ao ler o arquivo',false);};r.readAsArrayBuffer(f);}
  ...
  app.all("/api/tmp/fiscseller-6931", async (req, res) => {
    try {
      ...
      const sellerExpr = "COALESCE((SELECT NULLIF(TRIM(CONCAT(u.first_name,' ',u.last_name)),'') FROM sales_cards sc JOIN users u ON (u.omie_vendor_code=sc.seller_id OR u.omie_vendor_code=replace(COALESCE(sc.seller_id,''),'omie-vendor-','') OR u.id=sc.seller_id) WHERE sc.id=fi.sales_card_id LIMIT 1), (SELECT NULLIF(TRIM(CONCAT(u2.first_name,' ',u2.last_name)),'') FROM customers c JOIN users u2 ON (u2.omie_vendor_code=c.seller_id OR u2.omie_vendor_code=replace(COALESCE(c.seller_id,''),'omie-vendor-','') OR u2.id=c.seller_id) WHERE regexp_replace(COALESCE(fi.customer_cnpj_cpf,''),'[^0-9]','','g')<>'' AND regexp_replace(COALESCE(fi.customer_cnpj_cpf,''),'[^0-9]','','g')=regexp_replace(COALESCE(c.cnpj,c.cpf,''),'[^0-9]','','g') LIMIT 1), 'Sem vendedor')";
      const where = "FROM fiscal_invoices fi WHERE fi.status='authorized' AND COALESCE(fi.operation_type,'saida')<>'entrada' AND COALESCE(fi.fin_nfe,'1')<>'4' AND " + nat + " AND (fi.import_origin IS NULL OR TRIM(fi.import_origin)='') AND (COALESCE(fi.emission_date,fi.authorization_date,fi.created_at) AT TIME ZONE 'America/Sao_Paulo')::date >= date_trunc('month',(now() AT TIME ZONE 'America/Sao_Paulo'))::date";
      const perSeller = await q2("WITH fx AS (SELECT fi.total_invoice AS v, " + sellerExpr + " AS seller " + where + ") SELECT seller, COALESCE(SUM(v),0) AS total, COUNT(*) AS n FROM fx GROUP BY seller ORDER BY total DESC");
      const grand = await q2("SELECT COALESCE(SUM(fi.total_invoice),0) AS total, COUNT(*) AS n " + where);
