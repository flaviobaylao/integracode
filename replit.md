...
- **Customer Management**: Client-side search and filtering, customer inactivation. Customer details modal displays delivery configuration (days, time slots, vehicle types, exclusive vehicle) with color-coded badges. Prioritizes `fantasy_name` display. Unified customer modals with full feature parity for editing customer data, including robust normalization of delivery weekdays from legacy Omie data formats. **Automatic Delivery Days Calculation**: Delivery days are automatically calculated from visit days (2 next business days). Example: Monday visit → Tuesday + Wednesday delivery; Thursday visit → Friday + Monday delivery (skips weekends). Business rule enforced across all customer creation/update endpoints (PATCH/POST customers, Omie import, Omie sync). System admin page (`/admin/system`) provides dry-run and apply recalculation tools for bulk correction of existing customer data. **Terminology Distinction**: The system distinguishes between "Dias/Horários de Recebimento" (when customer accepts receiving goods - configured via checkboxes in deliveryTimeSlots/deliverySaturdayTimeSlots) and "Dias de Entrega" (deliveryWeekdays - preferred execution days calculated automatically as 2 business days after route day). All UI labels use "Recebimento" (receiving) for customer-configured preferences.
...
- **Omie ERP Integration**: Hourly synchronization of clients, vendors, products, overdue debts, and invoices. Order blocking system. Hotsite orders are sent to Omie ERP, with automatic customer creation and robust validation.
