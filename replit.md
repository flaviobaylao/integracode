...
- **Financial Tracking**: Overdue debt monitoring and credit analysis. Automatic synchronization of overdue debts from Omie ERP 3 times daily (6am, 12pm, 3pm Brazil time) using `data_previsao` field for accurate due date calculation and `valor_a_receber` for pending amounts. Debts are persisted in `overdue_debts` table for efficient order blocking without real-time API calls. **Contas a Receber Tab**: New comprehensive accounts receivable view that loads all títulos in a single API call and performs frontend filtering for maximum performance. Calculates vencidos using `data_previsao < today && valor_a_receber > 0`. Features include search by client/document, status filtering (all/vencidos/a_vencer), real-time statistics, and Excel export functionality.
