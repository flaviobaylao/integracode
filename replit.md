...
- **Sales Card Configuration**: Role-based propagation system for sales card configuration changes, including automatic recurrence change propagation.
  - **New Card Creation from Route (Nov 2025)**: When vendors click on a customer card in the daily route view, the system opens a NEW sales card form with pre-filled data (customer, date, time, seller). This allows vendors to create fresh sales without duplicating existing cards. Implemented via `handleEditCard` function that passes `initialValues` to SalesCardModal while keeping `editingCard=null` to trigger creation mode.
