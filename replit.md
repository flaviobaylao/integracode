...
- **Enhanced Delivery Management** (October 2025): Delivery Management page fully integrated with Omie billings. System now loads invoices with `invoice_stage='Aguardando Rota'` (89 orders) directly from `billings` table instead of sales cards. Implements intelligent customer matching using 3 strategies: (1) Omie customer code via `omie-client-{code}` pattern, (2) normalized CPF comparison (removes punctuation), (3) normalized CNPJ comparison. COALESCE fallbacks ensure data availability even when customer doesn't exist in CRM. Features: order selection, urgent delivery marking, vehicle configuration modal with DialogDescription for proper accessibility, manual order refresh button, and clear display of delivery time slots and exclusive vehicle configurations. Supports multi-vehicle route planning with real-time optimization. Modal dismiss bug fixed by adding DialogDescription component to all Dialog components.
...
- **Sales Card Configuration Replication** (October 2025): Automatic propagation system that replicates configuration changes to all future pending cards of the same customer. When a sales card is updated via PUT /api/sales-cards/:id, the `updateFutureCardsConfig()` function automatically applies the same configuration changes to all future pending cards (status='pending', scheduledDate > current card) of that customer. **Replicated Fields**: routeDay, recurrenceType, paymentMethod, deliveryWeekdays, deliveryTimeSlots, deliverySaturdayTimeSlots, boletoDays, exclusiveVehicle, vehicleTypes, customerLatitude, customerLongitude. **Implementation**: Uses Drizzle ORM operators (gt, ne, eq, and) for precise filtering. Logs show "✅ X cards futuros atualizados com as novas configurações" with count of updated cards. **Use Case**: Ensures consistent scheduling when customer requirements change - updating one card automatically updates entire future agenda. Tested and validated via end-to-end Playwright tests confirming replication across 2+ future cards.
- **Sales Card Bulk Import Enhancement** (October 2025): Fixed critical bug where Excel/CSV import was ignoring routeDay and periodicidade columns from spreadsheet, allocating all cards to same day. System now reads and validates these fields with priority: (1) Read from spreadsheet columns ("Dia da Rota", "Periodicidade"), (2) Normalize through expanded dictionaries supporting variations (e.g., "segunda-feira"/"segunda feira"→"segunda", "quinzenal"/"15 dias"→"quinzenal"), (3) Validate against canonical values only, (4) Fallback to calculated/customer values if unrecognized. **Supported Variations** - Days: segunda/segunda-feira/seg→segunda, terça/terca/ter→terca, etc. Periods: semanal/semanalmente/7 dias→semanal, quinzenal/15 dias→quinzenal, mensal/30 dias→mensal, bimestral/60 dias→bimestral. **Logging**: ✅ when values read from spreadsheet, ⚠️ when using fallback. Prevents invalid data persistence by rejecting unrecognized values.
