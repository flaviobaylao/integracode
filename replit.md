...
- **Sales Card Configuration**: Role-based propagation system for sales card configuration changes, including automatic recurrence change propagation.
  - **Auto-Duplicate on Route Card Click (Nov 2025)**: When vendors open a customer card from the daily route view, the system automatically duplicates the card before opening the edit modal. This creates a new independent card copy, preventing edits to finalized cards while preserving the complete sales history in `order_history`. Implemented via `handleEditCard` function that calls POST `/api/sales-cards/{id}/duplicate` and displays "Card duplicado" notification.
