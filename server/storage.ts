...
export class DatabaseStorage implements IStorage {
  ...
  async getChatConversations(): Promise<ChatConversation[]> {
    return await db.select().from(chatConversations).orderBy(desc(chatConversations.hasUnread), desc(chatConversations.lastMessageTime), desc(chatConversations.updatedAt));
  }
