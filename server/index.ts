...
function up(){var f=document.getElementById('file').files[0];if(!f){show('Selecione o arquivo .p12',false);return;}var btn=document.getElementById('btn');btn.disabled=true;btn.textContent='Enviando...';var r=new FileReader();r.onload=function(){try{var bytes=new Uint8Array(r.result);var bin='';for(var i=0;i<bytes.length;i++)bin+=String.fromCharCode(bytes[i]);var b64=btoa(bin);fetch('/api/admin/pix/cert',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pfxBase64:b64,password:document.getElementById('pass').value})}).then(function(x){return x.json();}).then(function(j){btn.disabled=false;btn.textContent='Enviar certificado';if(j.ok){show('Certificado salvo com sucesso ('+j.savedBytes+' bytes, senha: '+(j.hasPassword?'sim':'nao')+'). Agora avise o Claude para validar a emissao de PIX.',true);}else{show('Erro: '+(j.error||JSON.stringify(j)),false);}}).catch(function(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro de rede: '+e.message,false);});}catch(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro: '+e.message,false);}};r.onerror=function(){btn.disabled=false;btn.textContent='Enviar certificado';show('Falha ao ler o arquivo',false);};r.readAsArrayBuffer(f);}
  ...
  app.get("/api/dashboard2/full", async (_req, res) => {
    try {
      ...
      const _vf = "fi.status='authorized' AND COALESCE(fi.operation_type,'saida')<>'entrada' AND COALESCE(fi.fin_nfe,'1')<>'4' AND UPPER(COALESCE(fi.nature_of_operation,'')) NOT LIKE '%DEVOL%' AND UPPER(COALESCE(fi.nature_of_operation,'')) LIKE '%VENDA%' AND UPPER(COALESCE(fi.nature_of_operation,'')) NOT LIKE '%TROCA%' AND UPPER(COALESCE(fi.nature_of_operation,'')) NOT LIKE '%TRANSFER%' AND UPPER(COALESCE(fi.nature_of_operation,'')) NOT LIKE '%REMESSA%' AND UPPER(COALESCE(fi.nature_of_operation,'')) NOT LIKE '%BONIFICA%' AND UPPER(COALESCE(fi.nature_of_operation,'')) NOT LIKE '%AMOSTRA%' AND (fi.import_origin IS NULL OR TRIM(fi.import_origin)='')";
      const faturadosDia = await q2("SELECT COALESCE(NULLIF(TRIM(fi.customer_name),''),'-') AS customer_name, COALESCE(SUM(fi.total_invoice),0) AS total FROM fiscal_invoices fi WHERE " + _vf + " AND (COALESCE(fi.emission_date,fi.authorization_date,fi.created_at) AT TIME ZONE 'America/Sao_Paulo')::date = (now() AT TIME ZONE 'America/Sao_Paulo')::date GROUP BY 1 HAVING COALESCE(SUM(fi.total_invoice),0) > 0 ORDER BY total DESC");
      const faturadosSemana = await q2("SELECT COALESCE(NULLIF(TRIM(fi.customer_name),''),'-') AS customer_name, COALESCE(SUM(fi.total_invoice),0) AS total FROM fiscal_invoices fi WHERE " + _vf + " AND (COALESCE(fi.emission_date,fi.authorization_date,fi.created_at) AT TIME ZONE 'America/Sao_Paulo')::date >= date_trunc('week', (now() AT TIME ZONE 'America/Sao_Paulo'))::date GROUP BY 1 HAVING COALESCE(SUM(fi.total_invoice),0) > 0 ORDER BY total DESC");
