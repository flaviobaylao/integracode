...
- **Delivery & Route Optimization**: Scheduled daily route generation using Nearest Neighbor + 2-opt algorithm with OSRM API. Visual mapping, checkpoint registration, performance dashboards. Supports multi-vehicle route planning, check-in/check-out system, and checkpoint distance tracking. Includes automatic coordinate validation with warnings for suspicious distances and critical route lengths. Admin diagnostic tool available. **Route Allocation Logic (Nov 13, 2025)**: Two critical corrections implemented: (1) **serviceStartDate Semantics**: `serviceStartDate` is now correctly treated as contract start date, NOT last visit date. Routes filter customers where `routeDate >= serviceStartDate`, but `lastCompletedDate` remains undefined for new customers until first actual visit. This ensures customers appear on their first configured weekday after contract start. (2) **Single Source of Truth**: Removed duplicate validation `shouldVisitOnDateByPeriodicity()` in `planDailyRoute()`. System now uses single validation via `calculateNextVisitDate()` in `getCustomersForDate()` for consistent route allocation. Date comparison uses simple string comparison (YYYY-MM-DD) avoiding timezone conversion issues. Handles weekdays as JSON array or Drizzle array type. Detailed debug logging added for customer filtering diagnostics.
