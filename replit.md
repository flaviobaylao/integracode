...
- **Visit Schedule Management**: Implemented a permanent sales card architecture where each active customer has one reusable `sales_card`. All visit results are stored in `order_history`. `nextVisitDate` is calculated from customer's `weekdays` and `visitPeriodicity`. Updates to customer weekdays or periodicity trigger recalculation of `nextVisitDate`. Route generation queries for `isPermanent=true` and `nextVisitDate` within the current day using **Brazil timezone (America/Sao_Paulo)** for accurate date matching.
  ...
  - **Automatic Card Reset System (Nov 2025)**: Implemented automatic reset service (`permanentCardResetService.ts`) that triggers when permanent cards are completed. Workflow: (1) create `order_history` entry preserving all visit data, (2) calculate reset data clearing temporary fields, (3) update card with clean state. **Fields cleared**: products, saleValue, notes, completedDate, attendanceStartDate, all check-in/out data, delivery fields, telemarketing fields, and **critically all Omie/invoice identifiers** (omieOrderId, omieOrderNumber, omieSyncStatus, omieSentAt, omieErrorMessage, invoiceNumber) to prevent duplicates. **Fields preserved**: isPermanent flag, customer/seller linkage, recurrence configuration (weekdays, periodicity). System recalculates `nextVisitDate` and resets status to 'pending', making card ready for reuse.
...
- **Sales Card Configuration**: Role-based propagation system for sales card configuration changes, including automatic recurrence change propagation.
  - **Complete Order Duplication from Route (Nov 2025)**: When vendors click on a customer card in the daily route view, the system creates a complete duplicate of the last sales order including ALL products, quantities, prices, payment method, freight, and discount. Backend endpoint POST `/api/customers/:customerId/duplicate-last-order` fetches the last order via `storage.getLastCustomerOrder()`, creates a new sales card with all duplicated data, and returns the complete `SalesCardWithRelations`. Frontend opens `SalesCardDetailsModal` with the duplicated card ready for editing. If the customer has no previous orders, the system falls back to opening `SalesCardModal` with a blank form and displays a "Primeiro pedido" toast notification. This streamlines the sales process by eliminating manual data entry for repeat orders. Implemented via `handleEditCard` function that calls the duplication endpoint and handles both success and fallback scenarios with appropriate UI feedback.
...
- **Omie ERP Integration**: Hourly synchronization of clients, vendors, products, overdue debts, invoices, including product mapping and customer registration. Sync status tracking.
  - **Sales Category Code (Nov 2025)**: Fixed fiscal category `OMIE_SALES_CATEGORY_CODE = "1.01.01"` (Clientes - Venda de Mercadoria Fabricadas) used for all order submissions to Omie. This is a constant defined in `omieIntegration.ts` and is not user-configurable to ensure consistency across all sales transactions. Previously used incorrect code "1.01.02" (Serviços Prestados), corrected to "1.01.01" on Nov 10, 2025.
