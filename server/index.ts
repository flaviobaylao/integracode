...
function up(){var f=document.getElementById('file').files[0];if(!f){show('Selecione o arquivo .p12',false);return;}var btn=document.getElementById('btn');btn.disabled=true;btn.textContent='Enviando...';var r=new FileReader();r.onload=function(){try{var bytes=new Uint8Array(r.result);var bin='';for(var i=0;i<bytes.length;i++)bin+=String.fromCharCode(bytes[i]);var b64=btoa(bin);fetch('/api/admin/pix/cert',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pfxBase64:b64,password:document.getElementById('pass').value})}).then(function(x){return x.json();}).then(function(j){btn.disabled=false;btn.textContent='Enviar certificado';if(j.ok){show('Certificado salvo com sucesso ('+j.savedBytes+' bytes, senha: '+(j.hasPassword?'sim':'nao')+'). Agora avise o Claude para validar a emissao de PIX.',true);}else{show('Erro: '+(j.error||JSON.stringify(j)),false);}}).catch(function(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro de rede: '+e.message,false);});}catch(e){btn.disabled=false;btn.textContent='Enviar certificado';show('Erro: '+e.message,false);}};r.onerror=function(){btn.disabled=false;btn.textContent='Enviar certificado';show('Falha ao ler o arquivo',false);};r.readAsArrayBuffer(f);}
  ...
  app.all("/api/tmp/gilmar-6931", async (req, res) => {
    try {
      ...
      const q = "WITH gil AS (SELECT u.id::text AS uid, u.omie_vendor_code AS code FROM users u WHERE LOWER(TRIM(CONCAT(COALESCE(u.first_name,''),' ',COALESCE(u.last_name,'')))) LIKE 'gilmar%'), route AS (SELECT DISTINCT va.customer_id AS cid FROM visit_agenda va WHERE (va.scheduled_date AT TIME ZONE 'America/Sao_Paulo')::date = '" + D + "' AND (va.seller_id IN (SELECT uid FROM gil) OR va.seller_id IN (SELECT code FROM gil) OR va.seller_id IN (SELECT 'omie-vendor-'||code FROM gil)) UNION SELECT DISTINCT sc.customer_id AS cid FROM sales_cards sc WHERE (sc.scheduled_date AT TIME ZONE 'America/Sao_Paulo')::date = '" + D + "' AND (sc.seller_id IN (SELECT uid FROM gil) OR sc.seller_id IN (SELECT code FROM gil) OR sc.seller_id IN (SELECT 'omie-vendor-'||code FROM gil))), ord AS (SELECT bp.customer_id AS cid, MAX(bp.customer_name) AS name, COALESCE(SUM(bp.sale_value),0) AS v, COUNT(*) AS n FROM billing_pipeline bp WHERE (bp.created_at AT TIME ZONE 'America/Sao_Paulo')::date = '" + D + "' AND LOWER(TRIM(COALESCE(bp.seller_name,''))) LIKE 'gilmar%' AND COALESCE(bp.sale_value,0) > 0 AND LOWER(TRIM(COALESCE(bp.operation_type,'venda'))) = 'venda' AND NOT EXISTS (SELECT 1 FROM blocked_orders bo WHERE bo.sales_card_id = bp.sales_card_id AND bo.status='blocked') AND NOT EXISTS (SELECT 1 FROM fiscal_invoices fi WHERE fi.sales_card_id = bp.sales_card_id AND UPPER(COALESCE(fi.status,'')) LIKE '%CANCEL%') AND NOT EXISTS (SELECT 1 FROM fiscal_invoices fi2 WHERE fi2.sales_card_id = bp.sales_card_id AND " + natExcl + ") GROUP BY bp.customer_id) SELECT o.cid, o.name, o.v, o.n, (o.cid IN (SELECT cid FROM route)) AS on_route FROM ord o ORDER BY (o.cid IN (SELECT cid FROM route)) ASC, o.v DESC";
      const rows = await q2(q);
      const gil = await q2("SELECT u.id::text AS uid, u.omie_vendor_code AS code, TRIM(CONCAT(u.first_name,' ',u.last_name)) AS nome FROM users u WHERE LOWER(TRIM(CONCAT(COALESCE(u.first_name,''),' ',COALESCE(u.last_name,'')))) LIKE 'gilmar%'");
      res.json({ gil, rows });
  ...
  app.get("/api/dashboard2/full", async (_req, res) => {
    try {
      ...
      const q2 = async (text: string) => (await db.execute(sql.raw(text))).rows as any[];
      const statsRows = await q2(`SELECT COALESCE(SUM(total_invoice) FILTER (WHERE fdate = (now() AT TIME ZONE 'America/Sao_Paulo')::date), 0) AS today_sales, COALESCE(SUM(total_invoice) FILTER (WHERE fdate = (now() AT TIME ZONE 'America/Sao_Paulo')::date - 1), 0) AS yesterday_sales, COALESCE(SUM(total_invoice) FILTER (WHERE fdate = (now() AT TIME ZONE 'America/Sao_Paulo')::date - 7), 0) AS last_week_same_day_sales, COALESCE(SUM(total_invoice) FILTER (WHERE fdate >= date_trunc('week', (now() AT TIME ZONE 'America/Sao_Paulo'))::date), 0) AS week_sales, COALESCE(SUM(total_invoice) FILTER (WHERE fdate >= date_trunc('month', (now() AT TIME ZONE 'America/Sao_Paulo'))::date), 0) AS month_sales FROM (SELECT total_invoice, (COALESCE(emission_date, authorization_date, created_at) AT TIME ZONE 'America/Sao_Paulo')::date AS fdate FROM fiscal_invoices WHERE status='authorized' AND COALESCE(operation_type,'saida') <> 'entrada' AND COALESCE(fin_nfe,'1') <> '4' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%DEVOL%' AND UPPER(COALESCE(nature_of_operation,'')) LIKE '%VENDA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TROCA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TRANSFER%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%REMESSA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%BONIFICA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%AMOSTRA%') t`);
      ...
      const fiscalMonthRows = await q2(`SELECT COALESCE(SUM(total_invoice),0) AS v FROM fiscal_invoices WHERE status='authorized' AND COALESCE(operation_type,'saida') <> 'entrada' AND COALESCE(fin_nfe,'1') <> '4' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%DEVOL%' AND UPPER(COALESCE(nature_of_operation,'')) LIKE '%VENDA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TROCA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TRANSFER%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%REMESSA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%BONIFICA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%AMOSTRA%' AND (import_origin IS NULL OR TRIM(import_origin) = '') AND (COALESCE(emission_date,authorization_date,created_at) AT TIME ZONE 'America/Sao_Paulo') >= date_trunc('month', (now() AT TIME ZONE 'America/Sao_Paulo'))`);
      ...
      const dailySeriesRows = await q2(`SELECT (COALESCE(emission_date, authorization_date, created_at) AT TIME ZONE 'America/Sao_Paulo')::date::text AS d, COALESCE(SUM(total_invoice),0) AS v FROM fiscal_invoices WHERE status='authorized' AND COALESCE(operation_type,'saida') <> 'entrada' AND COALESCE(fin_nfe,'1') <> '4' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%DEVOL%' AND UPPER(COALESCE(nature_of_operation,'')) LIKE '%VENDA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TROCA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TRANSFER%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%REMESSA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%BONIFICA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%AMOSTRA%' AND (COALESCE(emission_date, authorization_date, created_at) AT TIME ZONE 'America/Sao_Paulo')::date >= date_trunc('month', (now() AT TIME ZONE 'America/Sao_Paulo'))::date GROUP BY d ORDER BY d`);
      ...
      const monthlySeriesRows = await q2(`SELECT to_char(date_trunc('month', (COALESCE(emission_date,authorization_date,created_at) AT TIME ZONE 'America/Sao_Paulo')), 'YYYY-MM') AS m, COALESCE(SUM(total_invoice),0) AS v FROM fiscal_invoices WHERE status='authorized' AND COALESCE(operation_type,'saida') <> 'entrada' AND COALESCE(fin_nfe,'1') <> '4' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%DEVOL%' AND UPPER(COALESCE(nature_of_operation,'')) LIKE '%VENDA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TROCA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%TRANSFER%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%REMESSA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%BONIFICA%' AND UPPER(COALESCE(nature_of_operation,'')) NOT LIKE '%AMOSTRA%' AND (import_origin IS NULL OR TRIM(import_origin) = '') AND (COALESCE(emission_date,authorization_date,created_at) AT TIME ZONE 'America/Sao_Paulo') >= date_trunc('year', (now() AT TIME ZONE 'America/Sao_Paulo')) GROUP BY m ORDER BY m`);
      ...
      const sellerDailyRows = await q2("SELECT COALESCE(NULLIF(TRIM(bp.seller_name),''), 'Sem vendedor') AS seller, (bp.created_at AT TIME ZONE 'America/Sao_Paulo')::date::text AS d, COALESCE(SUM(bp.sale_value),0) AS v FROM billing_pipeline bp WHERE (bp.created_at AT TIME ZONE 'America/Sao_Paulo')::date >= date_trunc('month', (now() AT TIME ZONE 'America/Sao_Paulo'))::date AND COALESCE(bp.sale_value,0) > 0 AND LOWER(TRIM(COALESCE(bp.operation_type,'venda'))) = 'venda' AND NOT EXISTS (SELECT 1 FROM blocked_orders bo WHERE bo.sales_card_id = bp.sales_card_id AND bo.status = 'blocked') AND NOT EXISTS (SELECT 1 FROM fiscal_invoices fi WHERE fi.sales_card_id = bp.sales_card_id AND UPPER(COALESCE(fi.status,'')) LIKE '%CANCEL%') AND NOT EXISTS (SELECT 1 FROM fiscal_invoices fi2 WHERE fi2.sales_card_id = bp.sales_card_id AND (UPPER(COALESCE(fi2.nature_of_operation,'')) LIKE '%DEVOL%' OR UPPER(COALESCE(fi2.nature_of_operation,'')) LIKE '%TROCA%' OR UPPER(COALESCE(fi2.nature_of_operation,'')) LIKE '%TRANSFER%' OR UPPER(COALESCE(fi2.nature_of_operation,'')) LIKE '%REMESSA%' OR UPPER(COALESCE(fi2.nature_of_operation,'')) LIKE '%BONIFICA%' OR UPPER(COALESCE(fi2.nature_of_operation,'')) LIKE '%AMOSTRA%')) GROUP BY 1, 2");
