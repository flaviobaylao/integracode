...
- **Exclusive Vehicle Delivery Configuration**: Sales cards and sale finalization screens support exclusive vehicle delivery configuration. Admin users (admin, coordinator, administrative roles) can mark deliveries as exclusive vehicle and select up to 2 vehicle types (truck/car/motorcycle). Configuration is available in both SalesCardModal (card creation/edit) and SaleModal (sale finalization). Interface includes checkbox to enable exclusive vehicle mode, vehicle type selectors with maximum 2 selections enforced via toast validation, and visual indicators with vehicle emojis (🚛🚗🏍️) in card details. Data is persisted in sales_cards table via exclusiveVehicle (boolean) and vehicleTypes (array) fields. Backend validation ensures data integrity during submission.
