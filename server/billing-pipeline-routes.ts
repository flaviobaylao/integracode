...
export function registerBillingPipelineRoutes(app: Express) {
  ...
  // RECUPERACAO DEFINITIVA (blindagem): recria QUALQUER pedido da loja que ficou sem card, a
  // partir do payload gravado em hotsite_order_intake no momento em que o pedido chegou (todos os
  // meios: boleto/direto, PIX, cartao). Pega: (a) intake 'received'/'failed' que nunca virou card;
  // (b) intake 'created' cujo card foi excluido depois. dryRun por padrao; {apply:true} recria via
  // /api/public/orders e envia ao pipeline. Idempotente. Filtro opcional por {phone}. minAgeMinutes
  // evita mexer em pedidos que acabaram de chegar (ainda em processamento).
  app.post('/api/admin/hotsite/recover-lost-orders', authenticateUser, isAdminOnly, async (req: any, res) => {
    try {
      const apply = req.body?.apply === true;
      const hoursRaw = Number(req.body?.hours);
      const hours = Number.isFinite(hoursRaw) && hoursRaw > 0 && hoursRaw <= 4320 ? Math.floor(hoursRaw) : 336;
      const minAgeRaw = Number(req.body?.minAgeMinutes);
      const minAgeMin = Number.isFinite(minAgeRaw) && minAgeRaw >= 0 && minAgeRaw <= 1440 ? Math.floor(minAgeRaw) : 15;
      const phoneDigits = String(req.body?.phone || '').replace(/\D/g, '');
      const last8 = phoneDigits.length >= 8 ? phoneDigits.slice(-8) : null;
      const INTERNAL_BASE = 'http://127.0.0.1:' + (process.env.PORT || '8080');
      const out: any = { apply, hours, minAgeMinutes: minAgeMin, phone: phoneDigits || null, lost: [], scanned: 0, recovered: 0 };

      try { await db.execute(sql.raw(`CREATE TABLE IF NOT EXISTS hotsite_order_intake (id varchar PRIMARY KEY DEFAULT gen_random_uuid(), order_number varchar, sales_card_id varchar, phone varchar, source varchar, status varchar NOT NULL DEFAULT 'received', payload text, error text, created_at timestamp DEFAULT now(), updated_at timestamp DEFAULT now())`)); } catch {}
      ...
      let rows: any[] = [];
      try {
        const r: any = await db.execute(sql.raw(`SELECT id, status, sales_card_id, order_number, phone, source, payload, error, created_at FROM hotsite_order_intake WHERE created_at > now() - interval '${hours} hours' AND created_at < now() - interval '${minAgeMin} minutes' ORDER BY created_at DESC LIMIT 1000`));
        rows = (r.rows || r) as any[];
      } catch (e: any) { return res.status(500).json({ error: 'hotsite_order_intake indisponivel: ' + (e?.message || String(e)) }); }

      for (const row of rows) {
        if (last8 && String(row.phone || '').slice(-8) !== last8) continue;
        out.scanned++;
        const hasCard = await cardExists(row.sales_card_id);
        if (hasCard) continue; // pedido esta OK
        // pedido perdido: nunca virou card, ou o card foi excluido depois
        let pl: any = null;
        try { pl = typeof row.payload === 'string' ? JSON.parse(row.payload) : row.payload; } catch {}
        const entry: any = {
          intakeId: row.id, status: row.status, order_number: row.order_number || null, created_at: row.created_at,
          reason: row.sales_card_id ? 'card_excluido' : (row.status === 'failed' ? 'criacao_falhou' : 'card_nunca_criado'),
          customer: pl?.customer ? { name: pl.customer.name, phone: pl.customer.phone, cpfCnpj: pl.customer.cpfCnpj } : null,
          items: (pl?.items || []).map((i: any) => `${i.productName} x${i.quantity}`), paymentMethod: pl?.paymentMethod, total: pl?.totalAmount, prevError: row.error || null,
        };
        if (!pl || !pl.items || !pl.items.length) { entry.skipped = 'payload sem itens'; out.lost.push(entry); continue; }
        if (apply) {
          try {
            const resp = await fetch(`${INTERNAL_BASE}/api/public/orders`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(pl) });
            const data: any = await resp.json();
            if (!resp.ok || !data?.orderId) throw new Error(data?.message || `HTTP ${resp.status}`);
            try { await db.execute(sql`UPDATE hotsite_order_intake SET sales_card_id = ${data.orderId}, order_number = ${data.orderNumber || row.order_number || null}, status = 'created', updated_at = now() WHERE id = ${row.id}`); } catch {}
            try { await db.execute(sql`UPDATE sales_cards SET notes = COALESCE(notes,'') || ${'\n[RECUPERADO] pedido da loja recriado por admin (intake ' + String(row.id) + ').'} WHERE id = ${data.orderId}`); } catch {}
            try { const rr = await reconcilePendingOrders({ apply: true, minAgeMinutes: 0, cardIds: [data.orderId] }); entry.pipeline = rr; } catch (e2: any) { entry.pipelineError = e2?.message || String(e2); }
            entry.recovered = { newOrderId: data.orderId, newOrderNumber: data.orderNumber };
            out.recovered++;
            console.log(`♻️ [RECOVER-LOST] Pedido recriado: ${data.orderNumber} (intake ${row.id})`);
          } catch (e: any) { entry.recoverError = e?.message || String(e); }
        } else { entry.wouldRecover = true; }
        out.lost.push(entry);
      }
      out.lostCount = out.lost.length;
      res.json({ ok: true, ...out });
    } catch (e: any) { res.status(500).json({ error: e?.message || String(e) }); }
  });
