...
- **Automated Agenda Management**: Automatic generation of future sales cards based on customer visit schedules and periodicity. Recursive card generation ensures continuous agenda coverage. Includes scheduled daily maintenance to ensure future programming for all clients. Initial card generation starts ~7 days in the future to allow planning time. For retroactive or near-future dates, use `generate-missing-monday-cards.ts` script (successfully created 492 cards for 20/10/2025).
