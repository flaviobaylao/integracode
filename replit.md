...
- **Inventory/Stock Management**: Comprehensive stock control at `/estoque` with lot tracking, per-instance stock (in-use and blocked), color-coded instance display, automatic stock transfers when in-use is depleted, and integration with NF-e billing flows (consume on emission, reverse on cancellation). Database tables: `inventory_lots`, `inventory_movements`. Admin-only write access, coordinator/administrative read access.
