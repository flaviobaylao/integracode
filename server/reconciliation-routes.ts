...
export function registerReconciliation(app: Express) {
  ...
  async function settlePayable(payId: string, amount: number, method: string, accountId: string | null, paidAtISO: string, by: string) {
    ...
    // FIX (teto): a baixa nao pode passar do que falta pagar. Pagavel de R$ 2.000 com
    // R$ 500 pagos, conciliado contra um debito de R$ 2.000, gravava amount_paid=2.500.
      ...
      for (const m of matches) {
        ...
        if (m.receivable_id) {
          ...
          if (rec) {
            ...
            // FIX: o DELETE antigo casava por (reference, amount) SEM LIMIT — duas
            // conciliacoes legitimas de mesmo valor sumiam ao desfazer UMA. E quando a
            // baixa tinha sido feita via boleto, a linha tem reference = nosso numero,
            // entao o DELETE nao achava nada: o amount_paid voltava a zero e o
            // pagamento ficava, deixando o titulo divergente para sempre.
            await db.execute(sql`
          ...
          if (pay) {
            ...
            // FIX: sem LIMIT, desfazer UMA conciliacao apagava TODAS as parcelas de
            // mesmo valor do pagavel.
            await db.execute(sql`
  ...
  app.post("/api/reconciliation/dedup-canonical", authenticateUser, requireRole(FIN_ROLES), async (req, res) => {
    try {
      ...
      if (dryRun) return res.json({ dryRun: true, porHorario, gruposDuplicados: gruposDup, linhasParaEspelhar: toMirror.length, gruposComBaixaDupla: comBaixaDupla, amostraBaixaDupla: gruposComBaixaDupla });
      let espelhados = 0;
